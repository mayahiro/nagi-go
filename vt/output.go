...
// ColorLevel is the terminal color encoding level
type ColorLevel uint8

const (
	// ColorIndexed256 emits indexed 256-color SGR sequences and is the zero-value baseline
	ColorIndexed256 ColorLevel = iota
	// ColorMonochrome suppresses non-default color output
	ColorMonochrome
	// ColorANSI16 emits only standard and bright ANSI colors
	ColorANSI16
	// ColorTrueColor emits 24-bit RGB SGR sequences
	ColorTrueColor
)

// Capabilities contains optional terminal encoder capabilities
type Capabilities struct {
	// ColorLevel is the maximum terminal color encoding level
	//
	// Its zero value preserves the indexed 256-color baseline.
...
func BaselineCapabilities() Capabilities {
	return Capabilities{ColorLevel: ColorIndexed256}
}

// ModernCapabilities returns all Phase 3 optional capabilities
func ModernCapabilities() Capabilities {
	return Capabilities{ColorLevel: ColorTrueColor, UnderlineColor: true, CursorShape: true, SynchronizedUpdates: true}
}
...
// KeyboardEnhancements is a Kitty keyboard protocol progressive-enhancement set
type KeyboardEnhancements uint8

const (
	// KeyboardDisambiguate reports ambiguous modified keys as escape sequences
	KeyboardDisambiguate KeyboardEnhancements = 1 << iota
	// KeyboardReportEventTypes reports press, repeat, and release actions
	KeyboardReportEventTypes
	// KeyboardReportAlternateKeys includes shifted and base-layout alternatives
	KeyboardReportAlternateKeys
	// KeyboardReportAllKeys reports every key as an escape sequence
	KeyboardReportAllKeys
	// KeyboardReportAssociatedText includes text generated by a key event
	KeyboardReportAssociatedText
	// NagiKeyboardEnhancements is the set used by the standard terminal runner
	NagiKeyboardEnhancements = KeyboardDisambiguate | KeyboardReportEventTypes |
		KeyboardReportAllKeys | KeyboardReportAssociatedText
)
...
const (
	...
	opRequestPrimaryDeviceAttributes
	opQueryKeyboardEnhancements
	opPushKeyboardEnhancements
	opPopKeyboardEnhancements
	opNextLine
	opSetStyle
	opResetStyle
	opWriteText
...
// RequestPrimaryDeviceAttributes returns a capability-query boundary operation
func RequestPrimaryDeviceAttributes() TerminalOp {
	return TerminalOp{kind: opRequestPrimaryDeviceAttributes}
}

// QueryKeyboardEnhancements returns an operation querying current Kitty flags
func QueryKeyboardEnhancements() TerminalOp {
	return TerminalOp{kind: opQueryKeyboardEnhancements}
}

// PushKeyboardEnhancements returns an operation pushing and replacing Kitty flags
func PushKeyboardEnhancements(enhancements KeyboardEnhancements) TerminalOp {
	return TerminalOp{kind: opPushKeyboardEnhancements, keyboardEnhancements: enhancements & 31}
}

// PopKeyboardEnhancements returns an operation popping the Kitty flag stack
func PopKeyboardEnhancements() TerminalOp {
	return TerminalOp{kind: opPopKeyboardEnhancements}
}
...
func appendEncodedOperation(
	...
	case opRequestPrimaryDeviceAttributes:
		output = append(output, "\x1B[c"...)
	case opQueryKeyboardEnhancements:
		output = append(output, "\x1B[?u"...)
	case opPushKeyboardEnhancements:
		output = append(output, "\x1B[>"...)
		output = strconv.AppendUint(output, uint64(operation.keyboardEnhancements), 10)
		output = append(output, 'u')
	case opPopKeyboardEnhancements:
		output = append(output, "\x1B[<u"...)
	case opNextLine:
...
func appendStyle(output []byte, style SgrStyle, capabilities Capabilities) []byte {
	capabilities.ColorLevel = normalizeColorLevel(capabilities.ColorLevel)
	...
	if capabilities.UnderlineColor && colorLevelRank(capabilities.ColorLevel) >= colorLevelRank(ColorIndexed256) {
...
func appendColor(output []byte, color SgrColor, prefix uint64, capabilities Capabilities) []byte {
	colorLevel := normalizeColorLevel(capabilities.ColorLevel)
	if color.Kind() == SgrColorDefault || colorLevel == ColorMonochrome {
		return output
	}
	capabilities.ColorLevel = colorLevel
...
func appendExtendedColor(output []byte, color SgrColor, prefix uint64, capabilities Capabilities) []byte {
	...
	case SgrColorIndexed:
		index, _ := color.Index()
		if colorLevelRank(capabilities.ColorLevel) >= colorLevelRank(ColorIndexed256) {
			appendParameter(prefix)
			appendParameter(5)
			appendParameter(uint64(index))
		} else {
			appendParameter(ansi16Code(prefix, indexToANSI16(index)))
		}
	case SgrColorRGB:
		...
		default:
			appendParameter(ansi16Code(prefix, rgbToANSI16(red, green, blue)))
		}
	}
	return output
}

func ansi16Code(prefix uint64, index uint8) uint64 {
	bright := index >= 8
	base := uint64(30)
	if prefix == 48 {
		base = 40
	...
	return base + uint64(index%8)
}

func indexToANSI16(index uint8) uint8 {
	switch {
	case index <= 15:
		return index
	case index <= 231:
		cube := index - 16
		return cubeToANSI16(cube/36, cube/6%6, cube%6)
	case index <= 243:
		return 0
	default:
		return 15
...
func rgbToANSI16(red, green, blue uint8) uint8 {
	return cubeToANSI16(boolLevel(red >= 128)*5, boolLevel(green >= 128)*5, boolLevel(blue >= 128)*5)
}

func cubeToANSI16(red, green, blue uint8) uint8 {
	bits := boolLevel(red >= 3) + 2*boolLevel(green >= 3) + 4*boolLevel(blue >= 3)
	bright := max(red, green, blue) >= 5
	return bits + 8*boolLevel(bright)
}
...
func normalizeColorLevel(level ColorLevel) ColorLevel {
	if level > ColorTrueColor {
		return ColorIndexed256
	}
	return level
}

func colorLevelRank(level ColorLevel) uint8 {
	switch normalizeColorLevel(level) {
